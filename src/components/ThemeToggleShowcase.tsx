...
const ThemeToggleShowcase = () => {
  ...
  if (!mounted) {
    return (
      <div className="flex h-[400px] w-full items-center justify-center">
        <div className="size-8 animate-spin rounded-full border-4 border-neon-blue border-t-transparent"></div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-8 py-16">
      ...
      >
        <motion.div layout>
          <ThemeToggleButton1 className={cn("size-14")} />
        </motion.div>
        <motion.div layout>
          <ThemeToggleButton2 className={cn("size-14 p-2")} />
        </motion.div>
        <motion.div layout>
          <ThemeToggleButton3 className={cn("size-14 p-2")} />
        </motion.div>
        <motion.div layout>
          <ThemeToggleButton4 className={cn("size-14 p-2")} />
        </motion.div>
        <motion.div layout>
          <ThemeToggleButton5 className={cn("size-14 p-3")} />
        </motion.div>
      </motion.div>
      ...
      <div className="text-center max-w-md">
        ...
        <p className="text-text-secondary text-sm leading-relaxed">
          Interactive theme toggle buttons with smooth animations powered by
          Framer Motion. All buttons are synchronized with your system theme.
        </p>
      </div>
    </div>
  );
...
export const ThemeToggleButton1 = ({
  ...
  return (
    <button
      type="button"
      className={cn(
        "rounded-full transition-all duration-300 active:scale-95 hover:shadow-lg hover:shadow-neon-blue/25 bg-background-secondary border border-glass-border/30",
        className,
      )}
...
export const ThemeToggleButton2 = ({
  ...
  return (
    <button
      type="button"
      className={cn(
        "rounded-full transition-all duration-300 active:scale-95 hover:shadow-lg hover:shadow-neon-purple/25 bg-background-secondary border border-glass-border/30",
        className,
      )}
      onClick={() => setTheme(isDark ? "light" : "dark")}
      aria-label="Toggle theme"
...
export const ThemeToggleButton3 = ({
  ...
  return (
    <button
      type="button"
      className={cn(
        "rounded-full transition-all duration-300 active:scale-95 hover:shadow-lg hover:shadow-neon-cyan/25 bg-background-secondary border border-glass-border/30",
        className,
      )}
      onClick={() => setTheme(isDark ? "light" : "dark")}
      aria-label="Toggle theme"
